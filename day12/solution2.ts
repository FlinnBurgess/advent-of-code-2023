...
for await (const row of rl) {
  ...
  let multipliedSprings: string = springs;
  let multipliedGoal: string = goal;
  for (let i = 0; i < 2; i++) {
    multipliedSprings = multipliedSprings.concat("?", springs);
    multipliedGoal = multipliedGoal.concat(",", goal);
  }
  rowBeingProcessed = multipliedSprings;
  configCountForRow = 0;
  console.log("Matching for ", multipliedGoal);
  console.log(multipliedSprings);

  const groupSizes = multipliedGoal.split(",").map((s) => Number.parseInt(s));

  recursivelyLookForGroups(multipliedSprings, groupSizes, "");
